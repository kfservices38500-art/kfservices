...
import ctaBgImg from "../assets/cta-demandedevis.svg";
...
const Home = () => {
  ...
  return <div>
      ...
      <section className="relative overflow-hidden bg-muted">
        {/* Background photo – matches image aspect ratio */}
            ...
            {serviceCards.map((card, i) => <ScrollReveal key={card.title} delay={i * 0.1}>
              ...
              boxShadow: "0 20px 40px -15px rgba(0,0,0,0.15)"
                  ...
                  <div className="gradient-red p-6 pb-5">
                    ...
                    <h3 className="text-white text-[15px] font-bold leading-tight whitespace-nowrap">{card.title}</h3>
                    {card.subtitle && <p className="text-white/70 text-xs font-medium mt-1">{card.subtitle}</p>}
                  </div>
                  {/* Benefits */}
            ...
            {departments.map((d, i) => <ScrollReveal key={d.code} delay={i * 0.04}>
                <Link to={`/realisations?dept=${d.code}`}>
                  ...
                  >
                    ...
                    <span className="relative text-3xl md:text-4xl font-black block leading-none transition-all duration-300 gradient-red-text group-hover-white-text">{d.code}</span>
